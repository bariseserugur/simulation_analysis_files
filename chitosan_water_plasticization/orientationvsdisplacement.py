...
def PW_HB_analysis(start_frames):
    ...
    for state in states:
        for category_index in range(len(orientation_categories[1:])):
            ...
            all_distances[state,category_index] = 0
    ...
    for start_frame in start_frames:
        ...
        for key,value in first_frame_HBs.items():
            for HBix,HB in enumerate(value):
                ...
                if np.any(consecutive_zeros) == False:
                    ...
                    break_frame = remaining_frames[np.where(mask == 1)][-1] #last time bond was observed
            ...
            for key,value in first_frame_HBs.items():
                for HBix,HB in enumerate(value):
                    if remaining_index == 0:
                        ...
                        first_position = frame_xyz[solvent_atom_ix[0]]
                        last_position = xyz_list[len(remaining_frames)-1][solvent_atom_ix[0]]
                        delta = last_position - first_position
                        delta = np.where(delta > 0.5*L, delta-L, np.where(delta < -0.5*L, delta+L, delta))
                        distance = np.linalg.norm(delta)
                        first_frame_distances[key].append(distance)
        ...
        for state,value in first_frame_HB_orientations.items():
            ...
            for HBix,HBori in enumerate(first_frame_HB_orientations[state]):
                ...
                all_distances[state,category_index] += first_frame_distances[state][HBix]
        f.close()
    ...
    return (population_acf,population_freqs,orientation_acf,orientation_freqs,orientation_states,all_distances)
...
for output_tuple in outputs:
    ...
    output_distances = output_tuple[5]
    ...
    for state in states:
        for category_index in range(len(orientation_categories[1:])):
            ...
            all_distances[state,category_index] += output_distances[state,category_index]
...
f_out = open('orientationvsdisplacement.out','w')
for state in states:
...
for state in states:
    for category_index in range(len(orientation_categories[1:])):
        ...
        f_out.write('distances["{}",{}] = {}\n'.format(state,category_index,all_distances[state,category_index]))
