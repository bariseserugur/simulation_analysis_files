...
def PW_HB_analysis(start_frames):
    ...
    for start_frame in start_frames:
        ...
        for row in HB_frame:
            ...
            category_index = [orientation_categories[i]<cos and orientation_categories[i+1]>=cos for i in range(0, len(orientation_categories)-1)].index(True)
        ...
        for key,value in first_frame_HBs.items():
            for HBix,HB in enumerate(value):
                ...
                this_orientation = first_frame_HB_orientations[key][HBix]
                category_index = [orientation_categories[i]<this_orientation and orientation_categories[i+1]>=this_orientation for i in range(0, len(orientation_categories)-1)].index(True)
                
                orientation_acf[category_index] += mask
