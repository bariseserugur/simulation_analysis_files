...
def main(argv): 
    ...
    while still_data and stillLog: 
        if frame >= nskip and (frame-nskip) % samp_freq == 0:
            ...
            def per_segment_fv(grid_chunk):
                free_neigh_counts = [0 for i in polymer_indices]
                for polymer_no in range(len(polymer_indices)):
                    print(polymer_no)
                    free_neigh_counts[polymer_no] += find_proximity(grid_chunk, all_polymer_info[polymer_no], box_info, 5) #proximity criteria of 5 Angstroms
                return free_neigh_counts

            def worker(grid, start, end, queue):
                queue.put(per_segment_fv(grid[start:end]))

            nproc = 96
            bsize = (len(grid) // nproc) + 1
            procs = [None] * nproc
            queue = Queue()
            for i in range(nproc):
                # figure out subarray limits
                start, end = i * bsize, min((i + 1) * bsize, len(grid))
                print(start,end)
                p = Process(target=worker, args=(grid, start, end, queue))
                p.start()
                procs[i] = p

            fv_per_segment = np.sum([np.array(queue.get()) for i in range(nproc)],axis=0)
            for p in procs: p.join()
        
            f = open('polymer_fv' + "_%d.txt".format(traj_file) % timestep, "a")
            f.write('{}\n'.format(list(fv_per_segment*(r_probe**3))))
            f.close()
            nlog +=1 
        else: 
            # skip frame
