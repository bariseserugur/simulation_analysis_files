...
def per_segment_fv(tuple_input):
    ...
    for polymer_no in range(len(polymer_indices)):
        print(polymer_no)
        free_neigh_counts[polymer_no] += find_proximity(grid_chunk, all_polymer_info[polymer_no], box_info, rprox) #proximity criteria of 5 Angstroms
...
def main(argv): 
    ...
    while still_data and stillLog: 
        if frame >= nskip and (frame-nskip) % samp_freq == 0:
            ...
            f_out = open('polymer_fv' + "_%d.txt".format(traj_file) % timestep, "w")
            f_out.close()
            ...
            #grid = grid[:1000] ###############BIG CHANGE HERE DELETE
            grids = split(grid,96)
            ...
            inputs = [(grids[i],polymer_indices,all_polymer_info,box_info,5) for i in list(range(96))]
            outputs = pool.map(per_segment_fv, inputs)
            outputs = np.sum([np.array(i) for i in outputs],axis=0)

            f_out = open('polymer_fv' + "_%d.txt".format(traj_file) % timestep, "a")
