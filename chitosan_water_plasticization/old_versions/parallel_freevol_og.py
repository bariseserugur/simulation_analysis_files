...
def create_parser():
    ...
    parser.add_argument('-samp_freq', default=5, help='Frequency (in terms of frames) to sample configurations/data. (default=100)')
    parser.add_argument('-max_samp', default=10, help='Maximum number of frames to sample from the trajectory file. (default=10)')
    parser.add_argument('-nskip', default=4, help='Number of frames to skip in trajectory file. (default=5000)')
...
def main(argv): 
    ...
    while still_data and stillLog: 
        if frame >= nskip and (frame-nskip) % samp_freq == 0:
            ...
            output_name = output_file + "_{}_{}.txt".format(timestep,core_number)
            ...
            all_overlaps = find_overlaps(r_probe, grid, all_atom_info, box_info)
