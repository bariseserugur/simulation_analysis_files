...
def create_parser():
    ...
    parser.add_argument('-samp_freq', default=500, help='Frequency (in terms of frames) to sample configurations/data. (default=100)')
...
def main(argv): 
    ...
    while still_data and stillLog: 
        if frame >= nskip and (frame-nskip) % samp_freq == 0:
            ...
            #reduce everything down to only polymers:
            for key,value in all_atom_info.items():
                all_atom_info[key] = value[:11030]
