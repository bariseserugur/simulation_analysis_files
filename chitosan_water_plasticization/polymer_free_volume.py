...
#since polymer free volume, we don't count volume occupied by water below
...
frame_range = [0] #np.arange(0,5000,500)
...
for frame_index,frame in enumerate(frame_range):
    ...
    parallel_outputs = Parallel(n_jobs=CORE_COUNT)(delayed(group_overlap_check)(xyz,L,R_PROBE,final_groups[i]) for i in range(CORE_COUNT))
    ...
    f_out = open('polymer_fvf{}.out'.format(frame),'wb')
