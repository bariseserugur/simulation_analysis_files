...
# all_water_oxygens = [i.index for i in topology.atoms if i.index > 11029 and i.element.symbol == 'O']
...
def get_autocorrelation_function(water_group):
    ...
    return (stacked_orientations, stacked_distances)
...
all_orientations = np.vstack([i[0] for i in outputs])
all_distances = np.hstack([i[1] for i in outputs])

f_out = open('orientational_relaxation.out','wb')
np.save(f_out,all_orientations)
