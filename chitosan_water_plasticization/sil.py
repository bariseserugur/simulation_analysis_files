...
split_frames = split(frames,48)
xyz_outputs = Parallel(n_jobs=48, backend='multiprocessing')(delayed(read_positions)(split_frames[i]) for i in range(48))

xyz_list = [item for sublist in xyz_outputs for item in sublist]
xyz_list = np.stack(xyz_list)

print('xyz_list',time.time()-sttime)
...
for start_frame in start_frames:
    ...
    print('start',time.time()-function_start)
    ...
    print('after',time.time()-function_start)
    # for state in states:
...
f_out = open('orientationvslifetime_v2.out','w')
for state in states:
